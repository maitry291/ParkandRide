...
class ZoneRulesBuilder {
    ...
    public ZoneRulesBuilder addRuleToWindow(int startYear, int endYear, Month month, int dayOfMonthIndicator, DayOfWeek dayOfWeek, LocalTime time, boolean timeEndOfDay, ZoneOffsetTransitionRule.TimeDefinition timeDefinition, int savingAmountSecs) {
        ...
        if (i < -28 || i > 31 || i == 0) {
            ...
            List<TZWindow> list = this.windowList;
            list.get(list.size() - 1).addRule(startYear, endYear, month, dayOfMonthIndicator, dayOfWeek, time, timeEndOfDay ? 1 : 0, timeDefinition, savingAmountSecs);
            return this;
        } else {
    ...
    public ZoneRulesBuilder addRuleToWindow(int startYear, int endYear, Month month, int dayOfMonthIndicator, DayOfWeek dayOfWeek, LocalTime time, int adjustDays, ZoneOffsetTransitionRule.TimeDefinition timeDefinition, int savingAmountSecs) {
        int i = dayOfMonthIndicator;
        Jdk8Methods.requireNonNull(month, "month");
        Jdk8Methods.requireNonNull(timeDefinition, "timeDefinition");
        ChronoField.YEAR.checkValidValue((long) startYear);
        ...
        if (i < -28 || i > 31 || i == 0) {
            ...
            List<TZWindow> list = this.windowList;
            list.get(list.size() - 1).addRule(startYear, endYear, month, dayOfMonthIndicator, dayOfWeek, time, adjustDays, timeDefinition, savingAmountSecs);
            return this;
        } else {
            throw new IllegalStateException("Must add a window before adding a rule");
        }
    ...
    class TZWindow {
        ...
        /* access modifiers changed from: package-private */
        public void addRule(int startYear, int endYear, Month month, int dayOfMonthIndicator, DayOfWeek dayOfWeek, LocalTime time, int adjustDays, ZoneOffsetTransitionRule.TimeDefinition timeDefinition2, int savingAmountSecs) {
            int endYear2;
            if (this.fixedSavingAmountSecs != null) {
                ...
                for (int year = startYear; year <= endYear2; year++) {
                    TZRule rule = new TZRule(year, month, dayOfMonthIndicator, dayOfWeek, time, adjustDays, timeDefinition2, savingAmountSecs);
                    if (lastRule) {
            ...
            if (this.lastRuleList.size() != 1) {
                if (this.windowEnd.equals(LocalDateTime.MAX)) {
                    ...
                    for (TZRule lastRule : this.lastRuleList) {
                        addRule(lastRule.year, this.maxLastRuleStartYear, lastRule.month, lastRule.dayOfMonthIndicator, lastRule.dayOfWeek, lastRule.time, lastRule.adjustDays, lastRule.timeDefinition, lastRule.savingAmountSecs);
                    ...
                    for (TZRule lastRule2 : this.lastRuleList) {
                        addRule(lastRule2.year, endYear + 1, lastRule2.month, lastRule2.dayOfMonthIndicator, lastRule2.dayOfWeek, lastRule2.time, lastRule2.adjustDays, lastRule2.timeDefinition, lastRule2.savingAmountSecs);
                    }
                    this.lastRuleList.clear();
    ...
    class TZRule implements Comparable<TZRule> {
        ...
        TZRule(int year2, Month month2, int dayOfMonthIndicator2, DayOfWeek dayOfWeek2, LocalTime time2, int adjustDays2, ZoneOffsetTransitionRule.TimeDefinition timeDefinition2, int savingAfterSecs) {
            this.year = year2;
            this.month = month2;
        ...
        public ZoneOffsetTransition toTransition(ZoneOffset standardOffset, int savingsBeforeSecs) {
            ...
            return new ZoneOffsetTransition((LocalDateTime) ZoneRulesBuilder.this.deduplicate(this.timeDefinition.createDateTime((LocalDateTime) ZoneRulesBuilder.this.deduplicate(LocalDateTime.of(((LocalDate) ZoneRulesBuilder.this.deduplicate(date)).plusDays((long) this.adjustDays), this.time)), standardOffset, wallOffset)), wallOffset, (ZoneOffset) ZoneRulesBuilder.this.deduplicate(ZoneOffset.ofTotalSeconds(standardOffset.getTotalSeconds() + this.savingAmountSecs)));
        }
        ...
        public ZoneOffsetTransitionRule toTransitionRule(ZoneOffset standardOffset, int savingsBeforeSecs) {
            ...
            return new ZoneOffsetTransitionRule(this.month, this.dayOfMonthIndicator, this.dayOfWeek, this.time, this.adjustDays, this.timeDefinition, standardOffset, trans.getOffsetBefore(), trans.getOffsetAfter());
        }
        ...
        public int compareTo(TZRule other) {
            ...
            long timeSecs1 = (long) (this.time.toSecondOfDay() + (this.adjustDays * 86400));
            long timeSecs2 = (long) (other.time.toSecondOfDay() + (other.adjustDays * 86400));
            if (timeSecs1 < timeSecs2) {
                return -1;
            }
            return timeSecs1 > timeSecs2 ? 1 : 0;
        }

        private LocalDate toLocalDate() {
            int i = this.dayOfMonthIndicator;
            if (i < 0) {
                LocalDate date = LocalDate.of(this.year, this.month, this.month.length(IsoChronology.INSTANCE.isLeapYear((long) this.year)) + 1 + this.dayOfMonthIndicator);
                DayOfWeek dayOfWeek2 = this.dayOfWeek;
                if (dayOfWeek2 != null) {
                    return date.with(TemporalAdjusters.previousOrSame(dayOfWeek2));
                }
                return date;
            }
            LocalDate date2 = LocalDate.of(this.year, this.month, i);
            DayOfWeek dayOfWeek3 = this.dayOfWeek;
            if (dayOfWeek3 != null) {
                return date2.with(TemporalAdjusters.nextOrSame(dayOfWeek3));
            }
            return date2;
        }
    }
